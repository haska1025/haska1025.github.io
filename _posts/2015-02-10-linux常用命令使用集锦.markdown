...
coherency\_line\_size
level
number\_of\_sets
physical\_line\_partition
shared\_cpu\_list
shared\_cpu\_map
size
type
ways\_of\_associativity
coherency\_line\_size记录的就是cpu cache line size。

(2) sysconf (\_SC_LEVEL1\_DCACHE\_LINESIZE)
...
cd  ~username  Changes the working directory to the home directory of username. For example, cd ~bob changes the directory tothe home directory of user bob.
