...
###修改Linux下最大open files 数
    ...
    All changes need to be made by the root user and users will need to log out and log back in before the changes will take effect. 
      ...
      * - nofile 2048 
      ...
      Both types of limits must be set before the change in the maximum number of open files will take effect. 
...
**2.20	linux进程间通信机制查询**
    ...
    可用的方式包括信号量、共享内存、消息队列、管道、信号（signal）、套接字等形式[2]。使用IPCS可以查看共享内存、信号量、消息队列的状态。
    ipcs
    ipcrm
